...
function [lat, lon, alt] = ecef2lla(X, Y, Z)
    % Use constants a, b, e2 from the outer scope
    global a b e2;  % Declare a, b, e2 as global variables

    % Compute the distance from the Z axis
    p = sqrt(X^2 + Y^2);  
    lon = atan2(Y, X);  % Longitude (correctly calculated here)
    
    % Initialize the latitude using an approximation
    lat = atan2(Z, p * (1 - e2));  % This gives an initial approximation
    
    % Iterate to refine the latitude (start with just one iteration for simplicity)
    max_iter = 10;  % Set maximum number of iterations
    for i = 1:max_iter
        sin_lat = sin(lat);     % Sine of latitude
        cos_lat = cos(lat);     % Cosine of latitude
        
        % Compute the altitude
        alt = p / cos_lat - a / sqrt(1 - e2 * sin_lat^2);
        
        % Update the latitude using the formula (refining with iterative method)
        lat = atan2(Z + e2 * b * sin_lat^3, p);
    end
...
% Example of calling ecef2lla for a specific point
X = 15708120.16;  % Example X coordinate in meters
Y = 23393408.91;  % Example Y coordinate in meters
Z = 14280078.65;  % Example Z coordinate in meters
