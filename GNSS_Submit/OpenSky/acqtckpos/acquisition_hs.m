...
% 
%     if SNR >= 9.36  % acquisition thredhold
...
% end
% 
% % Save the Acquired structure to a .mat file
% save('D:\GNSS\Acquired_Urban_5000.mat', 'Acquired');
% 
% end % end function

function Acquired = acquisition_hs(file, signal, acq)
% Purpose: Perform signal acquisition using parallel code phase search algorithm
% Fine frequency is found using long FFT.
% Inputs:
%   file        - parameters related to the data file to be processed, a structure
%   signal      - parameters related to signals, a structure
% Outputs:
%   Acquired    - acquisition results, i.e., space vehicle (SV), signal-to-noise
%                 ratio (SNR), Doppler, code delay, and fine carrier frequency
...
for svindex = 1:32
    ...
    codechipshift = ceil(signal.Fs / signal.codeFreqBasis);
    SNR = 10 * log10(peak^2 / (sum(correlation(fbin, [1:codePhase - codechipshift, codePhase + codechipshift:end]).^2) / length(correlation(fbin, [1:codePhase - codechipshift, codePhase + codechipshift:end]))));

    if SNR >= 16  % acquisition threshold
        Acquired.sv = [Acquired.sv svindex];
        Acquired.SNR = [Acquired.SNR SNR];
        Acquired.Doppler = [Acquired.Doppler Doppler];
        Acquired.codedelay = [Acquired.codedelay codePhase - 1];
        disp(['SV ', num2str(svindex), ' acquired!']);  % Debugging line
    end
end
