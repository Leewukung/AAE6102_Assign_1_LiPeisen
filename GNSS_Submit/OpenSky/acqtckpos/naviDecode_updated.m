function [ephemeris, ALLTckResult, for_prest, bits] = naviDecode_updated(Acquired, ALLTckResult)
%Purpose
...
for svindex = 1 : length(Acquired.sv)
    ...
    for_prest = struct('nav1', [], 'sfb1', []);  % Initialize the fields of for_prest
    ...
    %%
    % Ensure ephemeris is initialized correctly for each satellite
    if ~isfield(ephemeris(prn), 'updatetime')
        ephemeris(prn).updatetime = [];
    end
    if ~isfield(ephemeris(prn), 'updatetime_tow')
        ephemeris(prn).updatetime_tow = [];
    end
    ...
    for index = startms:msdatalength
        ...
        if (msdatalength-index) > 70 % 100 because C/N0
    ...
    for index = 8 : navidatalength
        ...
        if length(index : navidatalength) > 360 % 300 next subframe
            ...
            if abs(sum((NaviData(index-7:index).*preamble)))>5 && ...
                    ...
                    if flag == 1
                        ...
                        for idx = 1:num_sf
                            ...
                            if flag_sf(svindex,1)==1 & flag_sf(svindex,2)==1 & flag_sf(svindex,3)==1 & flag_sf(svindex,4)==1 & flag_sf(svindex,5)==1 ...
                                ...
                                % Debugging prints
                                disp(['updatetime for SV ' num2str(prn) ': ' num2str(ephemeris(prn).updatetime)]);
                                disp(['updatetime_tow for SV ' num2str(prn) ': ' num2str(ephemeris(prn).updatetime_tow)]);
...
% Ensure for_prest is returned as part of the function's output
fprintf('Returning for_prest structure\n');
return;
