...
%% 主滤波循环（直接使用原始伪距观测）
trajectory = zeros(size(navSolutionsCT.usrPos,1), 6);  % 存储所有 epoch 的状态（位置和速度）
for epoch = 1:size(pseudorange, 1)
    ...
    % 组合预测值：伪距和多普勒
    z_pred = [z_pred_pseudo; z_pred_doppler];  % 组合后为 11×1
...
%% 计算用户的标量速度（单位：m/s）
% 对每个 epoch，速度标量为速度向量的欧几里得范数
speed = sqrt(sum(velocity.^2, 2));  % 结果为 [n_epoch x 1] 向量

for i = 1:length(speed)
    fprintf('Epoch %d: Speed = %.2f m/s\n', i, speed(i));
end
...
%% 将速度标量数据保存为 CSV 文件
speed_table = array2table(speed, 'VariableNames', {'Speed'});
csv_speed_path = fullfile(folder_path, 'speed_data.csv');
